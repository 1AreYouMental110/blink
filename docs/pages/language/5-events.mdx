...
#### `Auth`
Names an auth rule for generated middleware. Register the rule at runtime with `blink.UseAuth(name, callback)`.
If an auth rule returns false, the incoming event is dropped before the listener runs.

```lua filename="server.luau" copy
blink.UseAuth("AlivePlayer", function(context)
    return context.Source.Character ~= nil
end)
```

#### Guard Overrides
Events can override runtime guardrails:
...
blink.MyEvent.FireGroup("RedTeam", 5)
```

Client-origin events also expose `Predict(...)`, which sends the event and returns a local prediction id that can be passed to `blink.Prediction.Ack(id)` or `blink.Prediction.Rollback(id)`.
